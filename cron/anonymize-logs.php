...
/* Hetzner cronjob settings
00 03 * * * /usr/bin/php83 /usr/home/jozapf/public_html/cron/contactform/anonymize-logs.php
*/
...
// --- Prefer an explicit APP_ENV_FILE or external secrets directory (SECRETS_DIR)
// If the deploy/cron environment provides APP_ENV_FILE or SECRETS_DIR, prefer
// loading that secrets file (e.g. E:/Secrets/jozapf-de/app.env) and inject
// values into the process environment. This allows secrets to live outside
// the repository and be used by the cron job.
$preloadedSecrets = [];
$appEnvFileFromEnv = getenv('APP_ENV_FILE') ?: null;
$secretsDir = getenv('SECRETS_DIR') ?: null;

$secretCandidates = [];
if ($appEnvFileFromEnv) { $secretCandidates[] = $appEnvFileFromEnv; }
if ($secretsDir) {
    $secretCandidates[] = rtrim($secretsDir, "\\/") . '/app.env';
    $secretCandidates[] = rtrim($secretsDir, "\\/") . '/.app.env';
}

// Also try webroot candidate paths (production .app.env in webroot)
$webrootCandidates = [];
$possibleWebroot = realpath(CRON_DIR . '/../../');
if ($possibleWebroot !== false) {
    $webrootCandidates[] = $possibleWebroot . '/.app.env';
    $webrootCandidates[] = $possibleWebroot . '/app.env';
    $webrootCandidates[] = $possibleWebroot . '/assets/php/.app.env';
    $webrootCandidates[] = $possibleWebroot . '/assets/php/app.env';
}

foreach (array_merge($secretCandidates, $webrootCandidates) as $cand) {
    if (file_exists($cand)) {
        $preloadedSecrets = loadEnv($cand);
        break;
    }
}

if (!empty($preloadedSecrets)) {
    // Inject into process environment if not already set
    foreach ($preloadedSecrets as $k => $v) {
        $proc = getenv($k);
        if ($proc === false || $proc === '') {
            putenv("{$k}={$v}");
            $_ENV[$k] = $v;
    ...
    // Note: we do not return here; .env.prod detection continues for path config
}
...
// Get PUBLIC_HTML and PROJECT_NAME: prefer process env (e.g. from app.env mounted via SECRETS_DIR
// or an APP_ENV_FILE), otherwise fall back to values parsed from .env.prod
$PUBLIC_HTML = getenv('CRON_PUBLIC_HTML') ?: ($env['CRON_PUBLIC_HTML'] ?? null);

// Get PROJECT_NAME from environment first, then .env.prod
$PROJECT_NAME = getenv('PROJECT_NAME') ?: ($env['PROJECT_NAME'] ?? null);
