...
<?php
/**
 * Test Script for Anonymization Cronjob (Final Hetzner Version)
 * 
 * Purpose: Verify the anonymization script works correctly before setting up cronjob
 * 
 * LOCATION: /usr/home/users/cron/contactform/test-anonymization.php
 * 
 * Usage:
 ...
 * 
 * @author JoZapf
 * @version 3.0.0 - Final version for Hetzner (correct paths)
 * @since 2025-10-06
 */

// ============================================================================
// CONFIGURATION - HETZNER STRUCTURE (jozapf user)
// ============================================================================

define('CRON_DIR', __DIR__);
define('USER_HOME', '/usr/home/users');
define('WEB_ROOT', '/usr/www/users/jozapf');
define('PROJECT_ROOT', WEB_ROOT . '/jozapf-de');
...
printHeader("ContactForm GDPR Anonymization - Test Script (Final)");

echo "PHP Version: " . COLOR_GREEN . PHP_VERSION . COLOR_RESET . "\n";
echo "User: " . COLOR_GREEN . get_current_user() . COLOR_RESET . "\n";
echo "Cron Directory: " . COLOR_GREEN . CRON_DIR . COLOR_RESET . "\n";
echo "Web Root: " . COLOR_GREEN . WEB_ROOT . COLOR_RESET . "\n";
echo "Project Root: " . COLOR_GREEN . PROJECT_ROOT . COLOR_RESET . "\n\n";
...
printHeader("TEST 0: Checking Hetzner Directory Structure");

if (is_dir(USER_HOME)) {
    printSuccess("User home directory exists: " . USER_HOME);
} else {
    printError("User home directory NOT FOUND: " . USER_HOME);
    $allTestsPassed = false;
}

if (is_dir(WEB_ROOT)) {
    printSuccess("Web root exists: " . WEB_ROOT);
} else {
    printError("Web root NOT FOUND: " . WEB_ROOT);
    $allTestsPassed = false;
}

if (is_dir(PROJECT_ROOT)) {
    printSuccess("Project root exists: " . PROJECT_ROOT);
} else {
    printError("Project root NOT FOUND: " . PROJECT_ROOT);
    printInfo("Expected: /usr/www/users/jozapf/jozapf-de/");
    printWarning("Check your directory structure!");
    $allTestsPassed = false;
}

if (is_dir(PHP_DIR)) {
    printSuccess("PHP directory exists: " . PHP_DIR);
} else {
    printError("PHP directory NOT FOUND: " . PHP_DIR);
    $allTestsPassed = false;
}

if (is_dir(CRON_DIR)) {
    printSuccess("Cron directory exists: " . CRON_DIR);
} else {
    printError("Cron directory NOT FOUND: " . CRON_DIR);
    $allTestsPassed = false;
}
...
$requiredDirs = [
    ...
    [PHP_DIR . '/data', 'Data directory (if exists)'],
];
...
foreach ($requiredDirs as [$path, $desc]) {
    if (is_dir($path)) {
        ...
        printWarning("{$desc} does not exist: {$path}");
    }
}
...
if ($allTestsPassed) {
    ...
    printInfo("2. Set up cronjob in Hetzner Console:");
    printInfo("   0 3 * * * /usr/bin/php83 /usr/home/users/cron/contactform/anonymize-logs.php");
    ...
    printInfo("4. Check if project paths are correct:");
    printInfo("   Project: /usr/www/users/jozapf/jozapf-de/");
    printInfo("   Cron:    /usr/home/users/cron/contactform/");
    echo "\n";
    exit(1);
}
