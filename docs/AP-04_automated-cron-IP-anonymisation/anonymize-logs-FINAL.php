 ...
 * 
 * LOCATION: /usr/home/users/cron/contactform/anonymize-logs.php
 * 
 * Usage:
...
if (!is_dir(PROJECT_ROOT)) {
    ...
    fwrite(STDERR, "Expected structure: /usr/www/users/jozapf/jozapf-de/\n");
    exit(1);
}

if (!is_dir(PHP_DIR)) {
    fwrite(STDERR, "ERROR: PHP directory not found: " . PHP_DIR . "\n");
    exit(1);
}
...
if (!file_exists($extendedLoggerPath)) {
    fwrite(STDERR, "FATAL: ExtendedLogger.php not found at: {$extendedLoggerPath}\n");
    exit(1);
}
...
try {
    ...
    if (!is_writable(LOG_DIR)) {
        ...
        logCronError("Try: chmod 755 " . LOG_DIR);
        exit(1);
    }
